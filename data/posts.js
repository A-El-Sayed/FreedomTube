...
const getVideosByUser = async (username) => {
    const postCollection = await posts();
    const postsArray = await postCollection.find({username: username.toLowerCase()}).toArray();
    return postsArray
};
...
module.exports = { getAllPosts, insertPost, deleteVideoByS3Name, getVideosByUser, renamePost};
